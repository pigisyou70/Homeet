...
public class newHaveHouseActivity extends AppCompatActivity implements EasyPermissions.PermissionCallbacks {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView( R.layout.activity_new_have_house );
        //狀態列顏色設定
...
//                Log.d("picUri",i+":"+photoPaths.get( i ));
                ...
                //bmp =BitmapFactory.decodeFile( photoPaths.get( i ) );
            } catch (Exception e) {
            ...
            StorageReference mStorageRef = mFirebaseStorage
                    .getReference()
                    .child("FindRoommate/HaveHouse")
                    .child("H"+objNum.toString())
            ...
            uploadTask.addOnFailureListener(new OnFailureListener() {
                ...
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText( newHaveHouseActivity.this,"照片出了點問題，再上傳一次看看吧！", Toast.LENGTH_LONG).show();
                }
...
//                    Log.d("uploading", progressPersentage+"%");
    ...
    private void moveInDateListen(){  //可遷入日
        moveInDate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // date picker dialog
                datePick = new DatePickerDialog( newHaveHouseActivity.this,
    ...
    public void update(View v){
        ...
        {
            Toast toast = Toast.makeText( newHaveHouseActivity.this, "請輸入*必填欄位", Toast.LENGTH_LONG);
            //顯示Toast
            toast.show();
        } else{
            //將地址 轉成經緯度s
            try{
                ...
                myRef.child( "H"+objNum.toString() ).child( "lat" ).setValue(String.valueOf(objlatlng[0]));
                myRef.child( "H"+objNum.toString() ).child( "lng" ).setValue(String.valueOf(objlatlng[1]));
                latlngisOkey =true;
                ...
                Toast.makeText( newHaveHouseActivity.this, "地址出了點問題，再上傳看看吧!", Toast.LENGTH_LONG).show();
                latlngisOkey=false;
            }
            if(latlngisOkey) {
                myRef.child( "H" + objNum.toString() ).child( "title" ).setValue( S_title );
                myRef.child( "H" + objNum.toString() ).child( "discript" ).setValue( S_discript );
                myRef.child( "H" + objNum.toString() ).child( "status" ).setValue( S_status );
                myRef.child( "H" + objNum.toString() ).child( "type" ).setValue( S_type );
                myRef.child( "H" + objNum.toString() ).child( "county" ).setValue( S_county );
                myRef.child( "H" + objNum.toString() ).child( "city" ).setValue( S_city );
                myRef.child( "H" + objNum.toString() ).child( "address" ).setValue( S_address );
                myRef.child( "H" + objNum.toString() ).child( "maxfloor" ).setValue( S_maxFloor );
                myRef.child( "H" + objNum.toString() ).child( "currentfloor" ).setValue( S_curFloor );
                myRef.child( "H" + objNum.toString() ).child( "price" ).setValue( S_price );
                myRef.child( "H" + objNum.toString() ).child( "deposit" ).setValue( S_deposit );
                myRef.child( "H" + objNum.toString() ).child( "moveInDate" ).setValue( S_moveInDate );
                myRef.child( "H" + objNum.toString() ).child( "size" ).setValue( S_size );
                myRef.child( "H" + objNum.toString() ).child( "updateTime" ).setValue( time );
                myRef.child( "H" + objNum.toString() ).child( "visible" ).setValue( visible );
                myRef.child( "H" + objNum.toString() ).child( "pattern" ).setValue( S_pattern );
                myRef.child( "H" + objNum.toString() ).child( "userPhone" ).setValue( phoneNumber );
                myRef.child( "H" + objNum.toString() ).child( "rentTime" ).setValue( S_rentTime );
                myRef.child( "H" + objNum.toString() ).child( "setting" ).setValue( setting );
                myRef.child( "H" + objNum.toString() ).child( "objNum" ).setValue( objNum );
                myRef.child( "H" + objNum.toString() ).child( "gender" ).setValue(gender);
                myRef.child( "H" + objNum.toString()).child( "genderLimit" ).setValue(S_genderLimit);

                //在members那邊新增 找室友PO文的物件編號
                memberRef.child( phoneNumber ).child( "FindRoommate/HaveHouse" ).child( "H" + objNum.toString() ).setValue( S_title );
                if (reservation.isChecked()) {
                    myRef.child( "H" + objNum.toString() ).child( "reservation" ).setValue( true );
                } else
                    myRef.child( "H" + objNum.toString() ).child( "reservation" ).setValue( false );
                priceIncludeValue();
                deviceValue();
                //相片處理 如果
                if (photoPaths != null) {
                    ...
                    myRef.child( "H"+objNum.toString() ).child( "picNumber" ).setValue(picNumber);
                    Toast.makeText( newHaveHouseActivity.this, "上傳成功!", Toast.LENGTH_LONG).show();
                    Intent intent =new Intent( newHaveHouseActivity.this, MainActivity.class );
                    ...
                    StorageReference mStorageRef = mFirebaseStorage
                            ...
                            .child("H"+objNum.toString());
                    mStorageRef.delete();
                    Toast.makeText( newHaveHouseActivity.this, "上傳失敗，請再試試!", Toast.LENGTH_LONG).show();
                }
    ...
    private void priceIncludeValue(){
        if(btnManage.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "manage" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "manage" ).setValue("false");
        }
        if(btnNet.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "Net" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "Net" ).setValue("false");
        }
        if(btnElec.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "Elec" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "Elec" ).setValue("false");
        }
        if(btnWater.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "Water" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "priceInclude" ).child( "Water" ).setValue("false");
        }
    ...
    public void patternOptionClick(View view){
        if(patternOptions!=null){
            ...
            imm.hideSoftInputFromWindow(newHaveHouseActivity.this.getCurrentFocus().getWindowToken(), 0);
            patternOptions.show();
        }
        ...
        floorOptions = new OptionsPickerView.Builder( this,new OptionsPickerView.OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int options2, int options3, View v) {
                if(options1!=0&&options2!=0){
                    ...
                    floor.setText( "" );
                    Toast.makeText( newHaveHouseActivity.this, "請選擇樓層!!", Toast.LENGTH_LONG).show();
                }
            }
    ...
    private void deviceValue(){
        if(washMachine.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "washMachine" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "washMachine" ).setValue("false");
        }
        if(refrig.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "refrig" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "refrig" ).setValue("false");
        }
        if(tv.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "tv" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "tv" ).setValue("false");
        }
        if(cable.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "cable" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "cable" ).setValue("false");
        }
        if(waterHeater.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "waterHeater" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "waterHeater" ).setValue("false");
        }
        if(internet.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "internet" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "internet" ).setValue("false");
        }
        if(gus.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "gus" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "gus" ).setValue("false");
        }
        if(bed.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "bed" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "bed" ).setValue("false");
        }
        if(wardrobe.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "wardrobe" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "wardrobe" ).setValue("false");
        }
        if(sofa.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "sofa" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "sofa" ).setValue("false");
        }
        if(tableChair.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "tableChair" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "tableChair" ).setValue("false");
        }
        if(airCondition.isSelected()){
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "airCondition" ).setValue("true");
        }else{
            myRef.child( "H"+objNum.toString() ).child( "device" ).child( "airCondition" ).setValue("false");
        }
    }
    public void reservationClick(View v){
        if(reservation.isChecked()){
            Intent intent = new Intent( newHaveHouseActivity.this, SetReservation.class);
            intent.putExtra( "objNum","H"+objNum);
